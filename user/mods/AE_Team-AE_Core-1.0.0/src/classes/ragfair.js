"use strict";
let settings = require(`../../mod.config.json`).settings;
let fleaConfig = settings.traderOverhaul.config;
...
function fleaLock(sessionID) {
    ...
    if (!fileIO.exist(`./user/mods/AE_Team-AE_Core-1.0.0/src/FleaLock/killtime.json`)) {
        fileIO.write(`./user/mods/AE_Team-AE_Core-1.0.0/src/FleaLock/killtime.json`, true, true, false);
    }

    //check the if the timer is active
    let usedTime = fileIO.readParsed(`./user/mods/AE_Team-AE_Core-1.0.0/src/FleaLock/killtime.json`);
    ...
    for (let item of pmcData.Inventory.items) {
        switch (item._tpl) {
            case fleaKeyId:
                ...
                fileIO.write(`./user/mods/AE_Team-AE_Core-1.0.0/src/FleaLock/killtime.json`, timenow, true, false); //set the timer
            case devfleakeyId:
                fleaOpen = true;
                return fleaOpen;
        }
...
function createOffer(template, onlyFunc, usePresets = true, sessionID) {
    ...
    if (Object.values(global._database.traders.ragfair.assort.items).filter(scheme => scheme._tpl == template || scheme._id == template).length == 0) { //.trader to .traders
    ...
    if (fleaConfig.fleaLock) {
        ...
        let itemid = template;
        let itemfound = itemlist.find(cv => cv.Id === itemid);
        let itemCat = itemfound;
        let catList = fileIO.readParsed("./user/mods/AE_Team-AE_Core-1.0.0/src/FleaLock/item_cat.json").Cat;
        if (!hasDevKey(sessionID)) {
            switch (itemfound != undefined) {
                case true:
                    itemCat = itemfound.ParentId; //get ParentId
                    let getCat = catList.find(ts => ts._id === itemCat) // find Cat._id from item_cat.json
                    if (getCat === undefined) {
                        logger.logError(`Item is in Item list, but not in Category list! _TPL: ${template} CAT._ID: ${itemCat}`);
                    }
    ...
    if (usePresets && preset_f.handler.hasPreset(template)) {
        ...
        for (let p of presets) {
            ...
            switch (fleaConfig.fleaLock) {
                case true:
                    switch (hasDevKey(sessionID)) {
                        case true:
