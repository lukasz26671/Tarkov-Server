  ...
  for (let botType in db.bots) {
    ...
    if (typeof db.bots[botType].difficulty != "undefined") {
      if (typeof db.bots[botType].difficulty.easy != "undefined")
        difficulty_easy = fileIO.readParsed(
          "./" + db.bots[botType].difficulty.easy,
        );
      if (typeof db.bots[botType].difficulty.normal != "undefined")
        difficulty_normal = fileIO.readParsed(
          "./" + db.bots[botType].difficulty.normal,
        );
      if (typeof db.bots[botType].difficulty.hard != "undefined")
        difficulty_hard = fileIO.readParsed(
          "./" + db.bots[botType].difficulty.hard,
        );
      if (typeof db.bots[botType].difficulty.impossible != "undefined")
        difficulty_impossible = fileIO.readParsed(
          "./" + db.bots[botType].difficulty.impossible,
        );
    }
    ...
    _database.bots[botType].appearance = fileIO.readParsed(
      "./" + db.bots[botType].appearance,
    );
    _database.bots[botType].chances = fileIO.readParsed(
      "./" + db.bots[botType].chances,
    );
    _database.bots[botType].experience = fileIO.readParsed(
      "./" + db.bots[botType].experience,
    );
    _database.bots[botType].generation = fileIO.readParsed(
      "./" + db.bots[botType].generation,
    );
    _database.bots[botType].health = fileIO.readParsed(
      "./" + db.bots[botType].health,
    );
    _database.bots[botType].inventory = {};
    for (const name in db.bots[botType].inventory) {
      _database.bots[botType].inventory[name] = fileIO.readParsed(
        "./" + db.bots[botType].inventory[name],
      );
    }
    _database.bots[botType].names = fileIO.readParsed(
      "./" + db.bots[botType].names,
    );
  }
}
...
function _load_HideoutData() {
  ...
  _database.hideout.areas = fileIO.readParsed(
    "./" + db.user.cache.hideout_areas,
  );
  if (typeof _database.hideout.areas.data != "undefined") {
    _database.hideout.areas = _database.hideout.areas.data;
  }

  _database.hideout.production = fileIO.readParsed(
    "./" + db.user.cache.hideout_production,
  );
  if (typeof _database.hideout.production.data != "undefined") {
    _database.hideout.production = _database.hideout.production.data;
  }

  _database.hideout.scavcase = fileIO.readParsed(
    "./" + db.user.cache.hideout_scavcase,
  );
  ...
  for (let id in _database.hideout.areas) {
    for (let id_stage in _database.hideout.areas[id].stages) {
      ...
      if (
        ...
        stage.constructionTime >
          _database.gameplay.hideout.productionTimeDivide_Areas
      ) {
        stage.constructionTime =
          stage.constructionTime /
          _database.gameplay.hideout.productionTimeDivide_Areas;
  ...
  for (let id in _database.hideout.production) {
    if (
      _database.hideout.production[id].productionTime != 0 &&
      _database.hideout.production[id].productionTime >
        _database.gameplay.hideout.productionTimeDivide_Production
    ) {
      _database.hideout.production[id].productionTime =
        _database.hideout.production[id].productionTime /
        _database.gameplay.hideout.productionTimeDivide_Production;
  ...
  for (let id in _database.hideout.scavcase) {
    if (
      _database.hideout.production[id].ProductionTime != 0 &&
      _database.hideout.production[id].ProductionTime >
        _database.gameplay.hideout.productionTimeDivide_ScavCase
    ) {
      _database.hideout.production[id].ProductionTime =
        _database.hideout.production[id].ProductionTime /
        _database.gameplay.hideout.productionTimeDivide_ScavCase;
...
function _load_QuestsData() {
  ...
  if (typeof _database.quests.data != "undefined")
    _database.quests = _database.quests.data;
}
function _load_CustomizationData() {
  _database.customization = fileIO.readParsed(
    "./" + db.user.cache.customization,
  );
  if (typeof _database.customization.data != "undefined")
    _database.customization = _database.customization.data;
}
function _load_LocaleData() {
  ...
  for (let lang in db.locales) {
    let menuFile = fileIO.exist(
      db.user.cache["locale_menu_" + lang.toLowerCase()],
    )
      ? db.user.cache["locale_menu_" + lang.toLowerCase()]
      : db.locales[lang].menu;
    ...
    _database.locales.global[lang] = fileIO.readParsed(
      "./" + db.user.cache["locale_" + lang.toLowerCase()],
    );
...
function _load_TradersData() {
  ...
  for (let traderID in db.traders) {
    ...
    _database.traders[traderID].base = fileIO.readParsed(
      "./" + db.traders[traderID].base,
    );
    _database.traders[traderID].categories = fileIO.readParsed(
      "./" + db.traders[traderID].categories,
    );
    _database.traders[traderID].base.sell_category =
      _database.traders[traderID].categories; // override trader categories
    _database.traders[traderID].assort = fileIO.readParsed(
      "./" + db.user.cache["assort_" + traderID],
    );
    if (typeof _database.traders[traderID].assort.data != "undefined")
      _database.traders[traderID].assort =
        _database.traders[traderID].assort.data;
    if (_database.traders[traderID].base.repair.price_rate === 0) {
      _database.traders[traderID].base.repair.price_rate = 100;
      _database.traders[traderID].base.repair.price_rate *=
        _database.gameplayConfig.trading.repairMultiplier;
      _database.traders[traderID].base.repair.price_rate -= 100;
    } else {
      _database.traders[traderID].base.repair.price_rate *=
        _database.gameplayConfig.trading.repairMultiplier;
      if (_database.traders[traderID].base.repair.price_rate == 0)
        _database.traders[traderID].base.repair.price_rate = -1;
    }
    if (_database.traders[traderID].base.repair.price_rate < 0) {
      _database.traders[traderID].base.repair.price_rate = -100;
    }
